...
export default function AgentLayout() {
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notificationData.ticket_id) {
      // If there's a ticket involved, navigate to the ticket page
      navigate(`/agent/tickets?ticketId=${notificationData.ticket_id}`);
    } else if (notificationData.customer_id) {
      // If there's a customer involved, navigate to the customer page
      navigate(`/agent/customers?customerId=${notificationData.customer_id}`);
    } else if (notificationData.call_id) {
      // If there's a call involved, navigate to the call center
      navigate(`/agent/calls?callId=${notificationData.call_id}`);
    } else {
      // Default fallback
      navigate("/agent");
    }
                          ...
                          >
                            <div className="flex items-start">
                              <div className="flex-1 min-w-0">
                                ...
                                {notificationData.ticket_name && (
                                  <div className="text-xs text-gray-600 mt-1 flex items-center">
                                    <span className="truncate">
                                      Ticket: {notificationData.ticket_name}
                                    </span>
                                  </div>
                                )}
